...
def crop_images(images_dir, images_output_dir, masks_dir, mask_suffix=None, masks_output_dir=None, margin=[0, 30, 30]):
    ...
    for mask in masks_list:
        ...
        if mask.endswith('.nii.gz'):
            ...
            # 裁剪对应的图像并保存
            if mask_suffix:  # 如果有指定后缀（用于 imagesTr）
...
def main():
    nnunet_raw_dir = '/home/gjh/code/U-Mamba/data/nnUNet_raw/Dataset141_MyoPS2024_coarse_seg_match_no0255_crop_label_umamba_stage5_epoch150_0911'
    
    imagesTr_dir = os.path.join(nnunet_raw_dir, 'imagesTr')
    labelsTr_dir = os.path.join(nnunet_raw_dir, 'labelsTr')
    
    # 处理训练集图像的裁剪
    crop_images(imagesTr_dir, imagesTr_dir, labelsTr_dir, mask_suffix=['_0000', '_0001', '_0002'], masks_output_dir=None)
